 ...
 *
 */
class Cmr_info extends CI_Controller {

    public function __construct(){
        parent::__construct();
        islogin();
        $this->load->model("zkadmin/cmr_mod");
    }
    ...
    public function index(){
		...
		$limit = 5;
		$start = ($page-1) * $limit;
		$count_all = $this->cmr_mod->count_all($search);
		
		//生成分页链接,分页配置在libraries文件夹里
		$this->load->library('pages');
		$config['base_url'] = base_url("zkadmin/cmr_info/index?keywords=".$search);
		$config['first_url'] = base_url("zkadmin/cmr_info/index?keywords=".$search."&per_page=1.html");
		...
		$data['all'] = $count_all;
		$data['list'] = $this->cmr_mod->get_list($search, $start, $limit);
...
//         $data['room_info'] = $this->db->get("room_info")->result_array();
		$this->load->view('zkadmin/cmr_list', $data);
    }
    ...
    public function del(){
        $CMR_ID = $this->input->post('CMR_ID');
        //未被使用的，才能删除
        $result = $this->db->where('CMR_ID', $CMR_ID)->where('DAU_TYPE', 1)->where('STATE !=', '使用中')->get('cmr_info')->row_array();
        if(empty($result)){
            alert('设备正被使用.');
        }
        $this->db->where('CMR_ID', $CMR_ID)->where('DAU_TYPE', 1)->delete('cmr_info');
        redirect('zkadmin/cmr_info');
    }
    ...
    public function detail() {
        ...
        $data['cmr'] = $this->db->where('CMR_ID', $cmr_id)->where('DAU_TYPE', 1)->get('cmr_info')->row_array();
        $this->load->view('zkadmin/cmr_detail', $data);
    }
