...
class Water_meter extends CI_Controller {
    ...
    public function __construct(){
        parent::__construct();
        islogin();
        $this->load->model("zkadmin/water_meter_mod");
    }
    ...
    public function index(){
		...
		$limit = 5;
		$start = ($page-1) * $limit;
		$count_all = $this->water_meter_mod->count_all($search);
		
		//生成分页链接,分页配置在libraries文件夹里
		$this->load->library('pages');
		$config['base_url'] = base_url("zkadmin/water_meter/index?keywords=".$search);
		$config['first_url'] = base_url("zkadmin/water_meter/index?keywords=".$search."&per_page=1.html");
		...
		$data['all'] = $count_all;
		$data['list'] = $this->water_meter_mod->get_list($search, $start, $limit);
		$data['keywords'] = $search;
		
		//冷水表
		$data['meter_info'] = $this->db->where('METER_TYPE', 3)->get('meter_info')->result_array();
		
		$this->load->view('zkadmin/water_meter_list', $data);
    }
    ...
    public function detail() {
        ...
        $this->load->view('zkadmin/water_meter_detail', $data);
    }
}
