...
// 		redirect("zkadmin/login");
        //页面架构用iframe的情况
        echo '<script>window.parent.location.href = "../zkadmin/login"</script>';
	}
}
...
function user_islogin(){
	...
	if($CI->session->userdata('user_islogin') == false){
		redirect("mapp/login");
	}
}
...
function get_info(){
    $CI =& get_instance();
    $data['dict'] = $CI->session->userdata('dict');
    $data['village'] = $CI->session->userdata('village');
    $data['building'] = $CI->session->userdata('building');
    $data['room'] = $CI->session->userdata('room');
    return $data;
}
...
function get_info_admin(){
    ...
    $data = $CI->db->where('id', $userid)->where('username', $username)
        ->where('role_id', $userrole)->get('admin_info')->row_array();
    return $data;
}

function get_partner(){
    $CI =& get_instance();
    $alipay = $CI->db->where('type', 1)->get('payment_info')->row_array();
